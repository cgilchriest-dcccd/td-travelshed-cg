...
## Site location
#site=pd.read_excel(path+'waterfront/input.xlsx',sheet_name='input',dtype=str)
...
#site=site[['siteid','parkid','direction','name','type','borough','lat','long','intlat','intlong','distance','walktime']]
#
...
#    time.sleep(0.1)
#site.to_excel(path+'waterfront/input.xlsx',sheet_name='input',index=False)
...
if __name__=='__main__':
    location=pd.read_excel(path+'waterfront/input.xlsx',sheet_name='input',dtype=str)
...
#    destination=location.loc[0:663,['id','parkid','name','direction','latlong']]
#    destination=location.loc[664:max(location.count())-1,['id','parkid','name','direction','latlong']]
...
#        df.to_csv(path+'waterfront/'+destination.loc[i,'id']+'wt.csv',index=True,header=True,na_rep=999)
...
#        ax.set_axis_off()
#        if destination.loc[i,'direction']=='in':
#            ax.set_title('Saturday Midday Transit Travel Time (Minutes) to '+destination.loc[i,'id'],fontdict={'fontsize':'16','fontweight':'10'})
#        elif destination.loc[i,'direction']=='out':
#            ax.set_title('Saturday Midday Transit Travel Time (Minutes) from '+destination.loc[i,'id'],fontdict={'fontsize':'16','fontweight':'10'})
...
#        fig.tight_layout()
#        fig.savefig(path+'waterfront/'+destination.loc[i,'id']+'wtbk.jpg', dpi=300)
...
#        cbar=fig.colorbar(sm,cax=cax)
#        fig.tight_layout()
#        fig.savefig(path+'waterfront/'+destination.loc[i,'id']+'wtct.jpg', dpi=300)
