...
#
## Site location
#site=pd.read_excel(path+'waterfront/weekend/input.xlsx',sheet_name='input',dtype=str)
...
#site=site[['siteid','parkid','direction','name','borough','type','lat','long','intlat','intlong','distance','walktime']]
#
...
#    time.sleep(0.1)
#site.to_excel(path+'waterfront/weekend/input.xlsx',sheet_name='input',index=False)
...
# Set typical day
typicaldate='2018/09/15'

# Create arrival time list
arrivaltimeinterval=10 # in minutes
arrivaltimestart='11:00:00'
arrivaltimeend='14:00:00'
...
if __name__=='__main__':
    location=pd.read_excel(path+'waterfront/weekend/input.xlsx',sheet_name='input',dtype=str)
    ...
    destination=location.loc[0:max(location.count())-1,['id','parkid','name','direction','latlong']]
...
#        df.to_csv(path+'waterfront/weekend/'+destination.loc[i,'id']+'wt.csv',index=True,header=True,na_rep=999)
    ...
    for i in destination.index:
        tp=pd.read_csv(path+'waterfront/weekend/'+destination.loc[i,'id']+'wt.csv',dtype=str)
    ...
    wtbk.to_file(path+'waterfront/weekend/wtbk.shp')
    wtbk=wtbk.drop('geometry',axis=1)
    wtbk.to_csv(path+'waterfront/weekend/wtbk.csv',index=False)
    ...
    wtct.to_file(path+'waterfront/weekend/wtct.shp')
    wtct=wtct.drop('geometry',axis=1)
    wtct.to_csv(path+'waterfront/weekend/wtct.csv',index=False)
    # Create map for each site
    wtbk=gpd.read_file(path+'waterfront/weekend/wtbk.shp')
    wtbk.crs={'init': 'epsg:4326'}
    wtct=gpd.read_file(path+'waterfront/weekend/wtct.shp')
    wtct.crs={'init': 'epsg:4326'}
    for i in destination['parkid'].unique():
        ...
        ax.set_axis_off()
        ax.set_title('Weekday AM Peak Transit Travel Time (Minutes) to '+destination.loc[destination['parkid']==i,'name'].unique()[0],fontdict={'fontsize':'16','fontweight':'10'})
        ...
        fig.tight_layout()
        fig.savefig(path+'waterfront/weekend/'+i+'bk.jpg', dpi=300)
        ...
        cbar=fig.colorbar(sm,cax=cax)
        fig.tight_layout()
        fig.savefig(path+'waterfront/weekend/'+i+'ct.jpg', dpi=300)
