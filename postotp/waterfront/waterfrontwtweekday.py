...
## Site location
#site=pd.read_excel(path+'waterfront/weekday/input.xlsx',sheet_name='input',dtype=str)
...
#    time.sleep(0.1)
#site.to_excel(path+'waterfront/weekday/input.xlsx',sheet_name='input',index=False)
#
#
#
...
if __name__=='__main__':
    location=pd.read_excel(path+'waterfront/weekday/input.xlsx',sheet_name='input',dtype=str)
...
#        df.to_csv(path+'waterfront/weekday/'+destination.loc[i,'id']+'wt.csv',index=True,header=True,na_rep=999)
    ...
    for i in destination.index:
        tp=pd.read_csv(path+'waterfront/weekday/'+destination.loc[i,'id']+'wt.csv',dtype=str)
    ...
    wtbk.to_file(path+'waterfront/weekday/wtbk.shp')
    wtbk=wtbk.drop('geometry',axis=1)
    wtbk.to_csv(path+'waterfront/weekday/wtbk.csv',index=False)
    ...
    wtct.to_file(path+'waterfront/weekday/wtct.shp')
    wtct=wtct.drop('geometry',axis=1)
    wtct.to_csv(path+'waterfront/weekday/wtct.csv',index=False)
    # Create map for each site
    wtbk=gpd.read_file(path+'waterfront/weekday/wtbk.shp')
    wtbk.crs={'init': 'epsg:4326'}
    wtct=gpd.read_file(path+'waterfront/weekday/wtct.shp')
    wtct.crs={'init': 'epsg:4326'}
    for i in destination['parkid'].unique():
        ...
        fig.tight_layout()
        fig.savefig(path+'waterfront/weekday/'+i+'bk.jpg', dpi=300)
        ...
        cbar=fig.colorbar(sm,cax=cax)
        fig.tight_layout()
        fig.savefig(path+'waterfront/weekday/'+i+'ct.jpg', dpi=300)
