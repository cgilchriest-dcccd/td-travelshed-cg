...
def travelshed(arrt):
    ...
    url+='&maxWalkDistance='+str(maxWalkDistance)+'&maxPreTransitTime='+str(maxPreTransitTime*60)+cutoff
...
## Multiprocessing travelshed function for Penn and GCT
#if __name__=='__main__':
#    location=pd.read_csv(path+'location/subway.csv')
#    location['id']=['SUBWAY'+str(x).zfill(3) for x in location['subwayid']]
...
#        df.to_csv(path+'pr/'+destination.loc[i,'id']+'.csv',index=True,header=True,na_rep=999)
#    print(datetime.datetime.now()-start)
#
## Multiprocessing travelshed function for subway
#if __name__=='__main__':
#    location=pd.read_csv(path+'location/subway.csv')
#    location['id']=['SUBWAY'+str(x).zfill(3) for x in location['subwayid']]
#    location['latlong']=[str(x)+','+str(y) for x,y in zip(location['intlat'],location['intlong'])]
...
#    print(datetime.datetime.now()-start)
#
## Multiprocessing travelshed function for rail
#if __name__=='__main__':
#    location=pd.read_csv(path+'location/rail.csv')
#    location['id']=['RAIL'+str(x).zfill(2) for x in location['railid']]
#    location['latlong']=[str(x)+','+str(y) for x,y in zip(location['intlat'],location['intlong'])]
#    destination=location.loc[0:max(location.count())-1,['id','latlong']]
#    for i in destination.index[27:]:
#        df=parallelize(arrivaltime,travelshed)
#        df['TTMEDIAN']=df.median(skipna=True,axis=1)
#        df=df['TTMEDIAN'].sort_index()
#        df.name=destination.loc[i,'id']
...
#    print(datetime.datetime.now()-start)
#
## Multiprocessing travelshed function for ferry
#if __name__=='__main__':
#    location=pd.read_csv(path+'location/ferry.csv')
#    location['id']=['FERRY'+str(x).zfill(2) for x in location['ferryid']]
#    location['latlong']=[str(x)+','+str(y) for x,y in zip(location['intlat'],location['intlong'])]
#    destination=location.loc[0:max(location.count())-1,['id','latlong']]
#    for i in destination.index:
#        df=parallelize(arrivaltime,travelshed)
...
#    print(datetime.datetime.now()-start)
#
## Multiprocessing travelshed function for xbus
#if __name__=='__main__':
#    location=pd.read_csv(path+'location/xbus.csv')
#    location['id']=['XBUS'+str(x).zfill(6) for x in location['xbusid']]
#    location['latlong']=[str(x)+','+str(y) for x,y in zip(location['intlat'],location['intlong'])]
#    destination=location.loc[0:max(location.count())-1,['id','latlong']]
#    for i in destination.index:
#        df=parallelize(arrivaltime,travelshed)
...
## Multiprocessing travelshed function for psa
#if __name__=='__main__':
#    location=pd.read_csv(path+'location/psa.csv')
#    location['id']=['PSA'+str(x).zfill(1) for x in location['psaid']]
#    location['latlong']=[str(x)+','+str(y) for x,y in zip(location['intlat'],location['intlong'])]
#    destination=location.loc[0:max(location.count())-1,['id','latlong']]
#    for i in destination.index:
#        df=parallelize(arrivaltime,travelshed)
...
# Multiprocessing travelshed function for adjustment
if __name__=='__main__':
    location=pd.read_csv(path+'location/adj.csv')
    location['id']=['ADJ'+str(x).zfill(2) for x in location['adjid']]
    location['latlong']=[str(x)+','+str(y) for x,y in zip(location['intlat'],location['intlong'])]
    destination=location.loc[0:max(location.count())-1,['id','latlong']]
    for i in destination.index:
        df=parallelize(arrivaltime,travelshed)
        ...
        df=df['TTMEDIAN'].sort_index()
        df.name=destination.loc[i,'id']
        df.to_csv(path+'allstation/pr/'+destination.loc[i,'id']+'.csv',index=True,header=True,na_rep=999)
    print(datetime.datetime.now()-start)
