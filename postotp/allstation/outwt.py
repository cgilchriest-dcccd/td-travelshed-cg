...
#        df.to_csv(path+'allstation/outbound/'+destination.loc[i,'id']+'wt.csv',index=True,header=True,na_rep=999)
...
#    for i in destination.index:
#        tp=pd.read_csv(path+'allstation/outbound/'+destination.loc[i,'id']+'wt.csv',dtype=str)
#        tp.iloc[:,1]=pd.to_numeric(tp.iloc[:,1])
#        wtbk=wtbk.merge(tp,on='blockid')
#    wtbk.to_file(path+'allstation/outbound/wtbk.shp')
#    wtbk=wtbk.drop('geometry',axis=1)
#    wtbk.to_csv(path+'allstation/outbound/wtbk.csv',index=False)
...
#    wtct.to_file(path+'allstation/outbound/wtct.shp')
#    wtct=wtct.drop('geometry',axis=1)
#    wtct.to_csv(path+'allstation/outbound/wtct.csv',index=False)
    # Create map for each site
    wtbk=gpd.read_file(path+'allstation/outbound/wtbk.shp')
    wtbk.crs={'init': 'epsg:4326'}
    wtct=gpd.read_file(path+'allstation/outbound/wtct.shp')
    wtct.crs={'init': 'epsg:4326'}
    for i in destination.index:
        ...
        fig.tight_layout()
        fig.savefig(path+'allstation/outbound/'+destination.loc[i,'id']+'wtbk.jpg', dpi=300)
        ...
        cbar=fig.colorbar(sm,cax=cax)
        fig.tight_layout()
        fig.savefig(path+'allstation/outbound/'+destination.loc[i,'id']+'wtct.jpg', dpi=300)
