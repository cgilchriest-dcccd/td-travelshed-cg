...
if __name__=='__main__':
    ...
    for i in destination.index:
        ...
        df.to_csv(path+'bmn2/'+destination.loc[i,'id']+'exwt.csv',index=True,header=True,na_rep=999)
    ...
    for i in destination.index:
        tp=pd.read_csv(path+'bmn2/'+destination.loc[i,'id']+'exwt.csv',dtype=str)
        tp.iloc[:,1]=pd.to_numeric(tp.iloc[:,1])
        wtbk=wtbk.merge(tp,on='blockid')
    wtbk.to_file(path+'bmn2/exwtbk.shp')
    wtbk=wtbk.drop('geometry',axis=1)
    wtbk.to_csv(path+'bmn2/exwtbk.csv',index=False)
    ...
    wtct.to_file(path+'bmn2/exwtct.shp')
    wtct=wtct.drop('geometry',axis=1)
    wtct.to_csv(path+'bmn2/exwtct.csv',index=False)
    # Create map for each site
    wtbk=gpd.read_file(path+'bmn2/exwtbk.shp')
    wtbk.crs={'init': 'epsg:4326'}
    wtct=gpd.read_file(path+'bmn2/exwtct.shp')
    wtct.crs={'init': 'epsg:4326'}
    for i in destination.index:
        ...
        fig.tight_layout()
        fig.savefig(path+'bmn2/'+destination.loc[i,'id']+'exwtbk.jpg', dpi=300)
        ...
        cbar=fig.colorbar(sm,cax=cax)
        fig.tight_layout()
        fig.savefig(path+'bmn2/'+destination.loc[i,'id']+'exwtct.jpg', dpi=300)
