...
if __name__=='__main__':
    ...
    for i in destination.index[6:]:
        ...
        df.to_csv(path+'bmn2/'+destination.loc[i,'id']+'expr.csv',index=True,header=True,na_rep=999)
    ...
    for i in destination.index:
        tp=pd.read_csv(path+'bmn2/'+destination.loc[i,'id']+'expr.csv',dtype=str)
        tp.iloc[:,1]=pd.to_numeric(tp.iloc[:,1])
        prbk=prbk.merge(tp,on='blockid')
    prbk.to_file(path+'bmn2/exprbk.shp')
    prbk=prbk.drop('geometry',axis=1)
    prbk.to_csv(path+'bmn2/exprbk.csv',index=False)
    ...
    prct.to_file(path+'bmn2/exprct.shp')
    prct=prct.drop('geometry',axis=1)
    prct.to_csv(path+'bmn2/exprct.csv',index=False)    
    # Create park and ride map for each site
    prbk=gpd.read_file(path+'bmn2/exprbk.shp')
    prbk.crs={'init': 'epsg:4326'}
    prct=gpd.read_file(path+'bmn2/exprct.shp')
    prct.crs={'init': 'epsg:4326'}
    for i in destination.index:
        ...
        fig.tight_layout()
        fig.savefig(path+'bmn2/'+destination.loc[i,'id']+'exprbk.jpg', dpi=300)
    ...
    wtprbk.to_file(path+'bmn2/exwtprbk.shp')
    wtprbk=wtprbk.drop('geometry',axis=1)
    wtprbk.to_csv(path+'bmn2/exwtprbk.csv',index=False)
    ...
    wtprct.to_file(path+'bmn2/exwtprct.shp')
    wtprct=wtprct.drop('geometry',axis=1)
    wtprct.to_csv(path+'bmn2/exwtprct.csv',index=False)
    # Create walk and transit + park and ride map for each site
    wtprbk=gpd.read_file(path+'bmn2/exwtprbk.shp')
    wtprbk.crs={'init': 'epsg:4326'}
    wtprct=gpd.read_file(path+'bmn2/exwtprct.shp')
    wtprct.crs={'init': 'epsg:4326'}
    for i in destination.index:
        # Create block level map
        ...
        cbar=fig.colorbar(sm,cax=cax)
        fig.tight_layout()
        fig.savefig(path+'bmn2/'+destination.loc[i,'id']+'exwtprbk.jpg', dpi=300)
        # Create tract level map
        ...
        cbar=fig.colorbar(sm,cax=cax)
        fig.tight_layout()
        fig.savefig(path+'bmn2/'+destination.loc[i,'id']+'exwtprct.jpg', dpi=300)
