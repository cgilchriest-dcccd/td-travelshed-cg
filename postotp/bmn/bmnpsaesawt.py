...
if __name__=='__main__':
    ...
    for i in destination.index:
        ...
        df.to_csv(path+'bmn/'+destination.loc[i,'id']+'psaesawt.csv',index=True,header=True,na_rep=999)
    ...
    for i in destination.index:
        tp=pd.read_csv(path+'bmn/'+destination.loc[i,'id']+'psaesawt.csv',dtype=str)
        tp.iloc[:,1]=pd.to_numeric(tp.iloc[:,1])
        wtbk=wtbk.merge(tp,on='blockid')
    wtbk.to_file(path+'bmn/psaesawtbk.shp')
    wtbk=wtbk.drop('geometry',axis=1)
    wtbk.to_csv(path+'bmn/psaesawtbk.csv',index=False)
    ...
    wtct.to_file(path+'bmn/psaesawtct.shp')
    wtct=wtct.drop('geometry',axis=1)
    wtct.to_csv(path+'bmn/psaesawtct.csv',index=False)
    # Create map for each site
    wtbk=gpd.read_file(path+'bmn/psaesawtbk.shp')
    wtbk.crs={'init': 'epsg:4326'}
    wtct=gpd.read_file(path+'bmn/psaesawtct.shp')
    wtct.crs={'init': 'epsg:4326'}
    for i in destination.index:
        ...
        fig.tight_layout()
        fig.savefig(path+'bmn/'+destination.loc[i,'id']+'psaesawtbk.jpg', dpi=300)
        ...
        cbar=fig.colorbar(sm,cax=cax)
        fig.tight_layout()
        fig.savefig(path+'bmn/'+destination.loc[i,'id']+'psaesawtct.jpg', dpi=300)
