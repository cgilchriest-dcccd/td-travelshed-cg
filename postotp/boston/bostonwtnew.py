...
# Site location
site=pd.read_excel(path+'boston/input.xlsx',sheet_name='input',dtype=str)
...
for i in site.index:
    ...
    time.sleep(0.1)
site.to_excel(path+'boston/input.xlsx',sheet_name='input',index=False)
...
if __name__=='__main__':
    location=pd.read_excel(path+'boston/input.xlsx',sheet_name='input',dtype=str)
    ...
    for i in destination.index:
        ...
        df.to_csv(path+'boston/'+destination.loc[i,'id']+'wt.csv',index=True,header=True,na_rep=999)
    ...
    for i in destination.index:
        tp=pd.read_csv(path+'boston/'+destination.loc[i,'id']+'wt.csv',dtype=str)
        tp.iloc[:,1]=pd.to_numeric(tp.iloc[:,1])
        wtbk=wtbk.merge(tp,on='blockid')
    wtbk.to_file(path+'boston/wtbk.shp')
    wtbk=wtbk.drop('geometry',axis=1)
    wtbk.to_csv(path+'boston/wtbk.csv',index=False)
    ...
    wtct.to_file(path+'boston/wtct.shp')
    wtct=wtct.drop('geometry',axis=1)
    wtct.to_csv(path+'boston/wtct.csv',index=False)
    # Create map for each site
    wtbk=gpd.read_file(path+'boston/wtbk.shp')
    wtbk.crs={'init': 'epsg:4326'}
    wtct=gpd.read_file(path+'boston/wtct.shp')
    wtct.crs={'init': 'epsg:4326'}
    for i in destination.index:
        ...
        fig.tight_layout()
        fig.savefig(path+'boston/'+destination.loc[i,'id']+'wtbk.jpg', dpi=300)
        ...
        cbar=fig.colorbar(sm,cax=cax)
        fig.tight_layout()
        fig.savefig(path+'boston/'+destination.loc[i,'id']+'wtct.jpg', dpi=300)
