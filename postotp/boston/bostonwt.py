...
# Multiprocessing res travelshed function for boston
if __name__=='__main__':
    location=pd.read_csv(path+'boston/boston.csv')
    location['id']=['SITE'+str(x).zfill(1) for x in location['Site']]
    ...
    for i in destination.index:
        ...
        df.to_csv(path+'boston/'+destination.loc[i,'id']+'.csv',index=True,header=True,na_rep=999)
