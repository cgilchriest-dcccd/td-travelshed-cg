...
# tract
# allstation
allstationlist=pd.read_excel(path+'edc/edc2/location2.xlsx',sheet_name='allstation',dtype=str)
df=pd.read_csv(path+'edc/allstationworkwtct.csv',dtype=str)
allstation=df[['tractid']+list(allstationlist['siteid'])]

# nyctract
nyctractlist=pd.read_excel(path+'edc/edc2/location2.xlsx',sheet_name='nyctract',dtype=str)
...
# regionalrail
regionalraillist=pd.read_excel(path+'edc/edc2/location2.xlsx',sheet_name='regionalrail',dtype=str)
...
wtct.to_file(path+'edc/edc2/tract.shp')
wtct=wtct.drop('geometry',axis=1)
wtct.to_csv(path+'edc/edc2/tract.csv',index=False)
...
allstation.to_csv(path+'edc/edc2/allstation.csv',index=False)
...
nyctract.to_csv(path+'edc/edc2/nyctract.csv',index=False)

# regionalrail
regionalraillist=pd.read_excel(path+'edc/edc2/location2.xlsx',sheet_name='regionalrail',dtype=str)
df=pd.read_csv(path+'edc/regionalworkwtbk.csv',dtype=str)
regionalrail=df[['blockid']+list(regionalraillist['siteid'])]
regionalrail.to_csv(path+'edc/edc2/regionalrail.csv',index=False)

# edc
allstation=pd.read_csv(path+'edc/edc2/allstation.csv',dtype=str)
nyctract=pd.read_csv(path+'edc/edc2/nyctract.csv',dtype=str)
regionalrail=pd.read_csv(path+'edc/edc2/regionalrail.csv',dtype=str)
edc=allstation.merge(regionalrail,on='blockid')
edc=edc.merge(nyctract,on='blockid')
for i in edc.columns[1:]:
    edc[i]=pd.to_numeric(edc[i])
edc.to_csv(path+'edc/edc2/block.csv',index=False)
...
wtbk.to_file(path+'edc/edc2/block.shp')

# gravity
gvbk=pd.read_csv(path+'edc/edc2/block.csv',dtype=str)
...
gvbk.to_csv(path+'edc/edc2/blockgravity.csv',index=True,na_rep='0')
gvbk=pd.read_csv(path+'edc/edc2/blockgravity.csv',dtype=str)
gvbk=gvbk.set_index('blockid')
...
locdetail=pd.read_excel(path+'edc/edc2/location2.xlsx',sheet_name='summary',dtype=str)
...
gravitybk.to_csv(path+'edc/edc2/edc2.csv',index=True)
