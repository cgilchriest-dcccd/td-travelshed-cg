...
#        df.to_csv(path+'regionalrailnew/inbound/'+destination.loc[i,'id']+'pr.csv',index=True,header=True,na_rep=999)
...
#    for i in destination.index:
#        tp=pd.read_csv(path+'regionalrailnew/inbound/'+destination.loc[i,'id']+'pr.csv',dtype=str)
#        tp.iloc[:,1]=pd.to_numeric(tp.iloc[:,1])
#        prbk=prbk.merge(tp,on='blockid')
#    prbk.to_file(path+'regionalrailnew/inbound/prbk.shp')
#    prbk=prbk.drop('geometry',axis=1)
#    prbk.to_csv(path+'regionalrailnew/inbound/prbk.csv',index=False)
...
#    prct.to_file(path+'regionalrailnew/inbound/prct.shp')
#    prct=prct.drop('geometry',axis=1)
#    prct.to_csv(path+'regionalrailnew/inbound/prct.csv',index=False)    
#    # Create park and ride map for each site
#    prbk=gpd.read_file(path+'regionalrailnew/inbound/prbk.shp')
#    prbk.crs={'init': 'epsg:4326'}
#    prct=gpd.read_file(path+'regionalrailnew/inbound/prct.shp')
#    prct.crs={'init': 'epsg:4326'}
#    for i in destination.index:
...
#        fig.tight_layout()
#        fig.savefig(path+'regionalrailnew/inbound/'+destination.loc[i,'id']+'prbk.jpg', dpi=300)
...
#        cbar=fig.colorbar(sm,cax=cax)
#        fig.tight_layout()
#        fig.savefig(path+'regionalrailnew/inbound/'+destination.loc[i,'id']+'prct.jpg', dpi=300)
    # Join walk and transit + park and ride travelsheds to block shapefile
    wtbk=pd.read_csv(path+'regionalrailnew/inbound/wtbk.csv',dtype=str)
    wtbk=wtbk.set_index('blockid')
    loclist=list(wtbk.columns)
    wtbk.columns=['WT'+str(x) for x in wtbk.columns]
    wtbk=wtbk.reset_index()
    prbk=pd.read_csv(path+'regionalrailnew/inbound/prbk.csv',dtype=str)
    ...
    wtprbk.to_file(path+'regionalrailnew/inbound/wtprbk.shp')
    wtprbk=wtprbk.drop('geometry',axis=1)
    wtprbk.to_csv(path+'regionalrailnew/inbound/wtprbk.csv',index=False)
    ...
    wtprct.to_file(path+'regionalrailnew/inbound/wtprct.shp')
    wtprct=wtprct.drop('geometry',axis=1)
    wtprct.to_csv(path+'regionalrailnew/inbound/wtprct.csv',index=False)
    # Create walk and transit + park and ride map for each site
    wtprbk=gpd.read_file(path+'regionalrailnew/inbound/wtprbk.shp')
    wtprbk.crs={'init': 'epsg:4326'}
    wtprct=gpd.read_file(path+'regionalrailnew/inbound/wtprct.shp')
    wtprct.crs={'init': 'epsg:4326'}
    for i in destination.index:
        # Create block level map
        ...
        cbar=fig.colorbar(sm,cax=cax)
        fig.tight_layout()
        fig.savefig(path+'regionalrailnew/inbound/'+destination.loc[i,'id']+'wtprbk.jpg', dpi=300)
        # Create tract level map
        ...
        cbar=fig.colorbar(sm,cax=cax)
        fig.tight_layout()
        fig.savefig(path+'regionalrailnew/inbound/'+destination.loc[i,'id']+'wtprct.jpg', dpi=300)
