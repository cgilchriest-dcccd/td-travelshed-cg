...
def travelshed(arrt):
    bk=bkpt.copy()
    url='http://localhost:8801/otp/routers/default/isochrone?batch=true&mode=WALK,SUBWAY'
...
# Multiprocessing travelshed function for Penn Station and Grand Central (Subway)
if __name__=='__main__':
    location=pd.read_csv(path+'location/subway.csv')
    location['id']=['SUBWAY'+str(x).zfill(2) for x in location['subwayid']]
