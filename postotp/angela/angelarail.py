...
def travelshed(arrt):
    bk=bkpt.copy()
    url='http://localhost:8801/otp/routers/default/isochrone?batch=true&mode=CAR,WALK,RAIL'
...
# Multiprocessing travelshed function for Penn Station and Grand Central (Rail)
if __name__=='__main__':
    location=pd.read_csv(path+'location/subway.csv')
    location['id']=['RAIL'+str(x).zfill(2) for x in location['subwayid']]
    ...
    for i in destination.index[[131,423]]:
