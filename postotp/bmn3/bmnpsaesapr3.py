...
if __name__=='__main__':
    ...
    for i in destination.index:
        ...
        df.to_csv(path+'bmn3/'+destination.loc[i,'id']+'psaesapr.csv',index=True,header=True,na_rep=999)
    ...
    for i in destination.index:
        tp=pd.read_csv(path+'bmn3/'+destination.loc[i,'id']+'psaesapr.csv',dtype=str)
        tp.iloc[:,1]=pd.to_numeric(tp.iloc[:,1])
        prbk=prbk.merge(tp,on='blockid')
    prbk.to_file(path+'bmn3/psaesaprbk.shp')
    prbk=prbk.drop('geometry',axis=1)
    prbk.to_csv(path+'bmn3/psaesaprbk.csv',index=False)
    ...
    prct.to_file(path+'bmn3/psaesaprct.shp')
    prct=prct.drop('geometry',axis=1)
    prct.to_csv(path+'bmn3/psaesaprct.csv',index=False)    
    # Create park and ride map for each site
    prbk=gpd.read_file(path+'bmn3/psaesaprbk.shp')
    prbk.crs={'init': 'epsg:4326'}
    prct=gpd.read_file(path+'bmn3/psaesaprct.shp')
    prct.crs={'init': 'epsg:4326'}
    for i in destination.index:
        ...
        fig.tight_layout()
        fig.savefig(path+'bmn3/'+destination.loc[i,'id']+'psaesaprbk.jpg', dpi=300)
        ...
        cbar=fig.colorbar(sm,cax=cax)
        fig.tight_layout()
        fig.savefig(path+'bmn3/'+destination.loc[i,'id']+'psaesaprct.jpg', dpi=300)
    # Join walk and transit + park and ride travelsheds to block shapefile
    wtbk=pd.read_csv(path+'bmn3/psaesawtbk.csv',dtype=str)
    wtbk=wtbk.set_index('blockid')
    wtbk.columns=['WT'+str(x) for x in wtbk.columns]
    wtbk=wtbk.reset_index()
    prbk=pd.read_csv(path+'bmn3/psaesaprbk.csv',dtype=str)
    ...
    wtprbk.to_file(path+'bmn3/psaesawtprbk.shp')
    wtprbk=wtprbk.drop('geometry',axis=1)
    wtprbk.to_csv(path+'bmn3/psaesawtprbk.csv',index=False)
    ...
    wtprct.to_file(path+'bmn3/psaesawtprct.shp')
    wtprct=wtprct.drop('geometry',axis=1)
    wtprct.to_csv(path+'bmn3/psaesawtprct.csv',index=False)
    # Create walk and transit + park and ride map for each site
    wtprbk=gpd.read_file(path+'bmn3/psaesawtprbk.shp')
    wtprbk.crs={'init': 'epsg:4326'}
    wtprct=gpd.read_file(path+'bmn3/psaesawtprct.shp')
    wtprct.crs={'init': 'epsg:4326'}
    for i in destination.index:
        # Create block level map
        ...
        cbar=fig.colorbar(sm,cax=cax)
        fig.tight_layout()
        fig.savefig(path+'bmn3/'+destination.loc[i,'id']+'psaesawtprbk.jpg', dpi=300)
        # Create tract level map
        ...
        cbar=fig.colorbar(sm,cax=cax)
        fig.tight_layout()
        fig.savefig(path+'bmn3/'+destination.loc[i,'id']+'psaesawtprct.jpg', dpi=300)
